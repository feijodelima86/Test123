...
ALL.SUM<-data.frame(aggregate(as.numeric(dfALL[,n1]) ~ SAMPLE_DESCRIPTOR, dfALL, mean))
ALL.SUM[,3]<-aggregate(as.numeric(dfALL[,n1]) ~ SAMPLE_DESCRIPTOR, dfALL, se)[,2]

#ALL.SUM <- subset(ALL.SUM, SITE == ssite)

names(ALL.SUM)<-c("SAMPLE_DESCRIPTOR","MEAN.ALL","STDER.ALL")
ALL.SUM[,4]<--2
